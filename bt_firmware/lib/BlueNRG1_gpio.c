...
void GPIO_Init(GPIO_InitType* GPIO_InitStruct) {
	...
	assert_param(
			((GPIO_InitStruct->GPIO_Pin & GPIO_Pin_12) != GPIO_Pin_12) || (GPIO_InitStruct->GPIO_Mode != GPIO_Output));
	assert_param(
			((GPIO_InitStruct->GPIO_Pin & GPIO_Pin_13) != GPIO_Pin_13) || (GPIO_InitStruct->GPIO_Mode != GPIO_Output));
	...
	for (uint8_t i = 0; i < GPIO_PIN_COUNT; i++) {
		...
		if (tmp != 0) {
			...
			if (i < 8) {
				MODIFY_REG(GPIO->MODE0, (0xF << (i * 4)),
						(gpio_mode << (i * 4)));
			} else if ((i > 7) && (i < 16)) {
				MODIFY_REG(GPIO->MODE1, (0xF << ((i - 8) * 4)),
						(gpio_mode << ((i - 8) * 4)));
			}
