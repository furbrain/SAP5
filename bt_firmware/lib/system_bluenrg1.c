...
void Wait_LS_Stabilization(void) {
	...
	while ((ls_state != LS_STABILIZATION_ENDED)
			&& (number_iteration <= MAX_NUMBER_ITERATION)) {
...
void Set_RF_FrontEnd(void) {
	...
	if ((i + 3) < HOT_TABLE_RADIO_SIZE) {
		...
		BLUE_CTRL->RADIO_CONFIG = 0x10000U
				| (uint16_t) ((uint32_t) (&hot_table_radio_config[i])
						& 0x0000FFFFU);
		while ((BLUE_CTRL->RADIO_CONFIG & 0x10000) != 0)
			;
	}
...
void SET_BORconfigStatus(uint8_t enabled) {
	...
	if (enabled) {
		BLUE_CTRL->RADIO_CONFIG = 0x10000U
				| (uint16_t) ((uint32_t) BOR_config & 0x0000FFFFU);
		while ((BLUE_CTRL->RADIO_CONFIG & 0x10000) != 0)
			;
	} else {
...
__STATIC_INLINE void nextRF_transaction(uint16_t reset_reason) {
	...
	if (((Device_partInfo.die_major << 4) | (Device_partInfo.die_cut))
			< WA_DEVICE_VERSION) {
		load_hot_table = 0;
	}
...
#error "No definition for BOR Configuration"
		...
		BLUE_CTRL->RADIO_CONFIG = 0x10000U
				| (uint16_t) ((uint32_t) cold_start_config & 0x0000FFFFU);
		while ((BLUE_CTRL->RADIO_CONFIG & 0x10000) != 0)
			;
