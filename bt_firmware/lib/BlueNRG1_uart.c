...
void UART_Init(UART_InitType* UART_InitStruct) {
	...
	assert_param(
			IS_UART_HW_FLOW_CONTROL(UART_InitStruct->UART_HardwareFlowControl));
	assert_param(IS_FUNCTIONAL_STATE(UART_InitStruct->UART_FifoEnable));

	/*---------------------------- UART BaudRate Configuration -----------------------*/

	if (UART->CR_b.OVSFACT == 0)
		divider = (UART_CLOCK << 7)
				/ (UART_CLOCK_CYCLE16 * UART_InitStruct->UART_BaudRate);
	else
		divider = (UART_CLOCK << 7)
				/ (UART_CLOCK_CYCLE8 * UART_InitStruct->UART_BaudRate);
...
void UART_RXTimeoutConfig(uint32_t UART_TimeoutMS) {
	...
	tmp32 = (UART->IBRD) << 4;
	if (UART->CR_b.OVSFACT == 0)
		timeout_val = (UART_TimeoutMS * 16 * 16000)
				/ (((UART->FBRD) >> 2) + tmp32);
	else
		timeout_val = (UART_TimeoutMS * 8 * 16000)
				/ (((UART->FBRD) >> 3) + tmp32);
