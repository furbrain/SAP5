 ...
 * @return System Time Units
 */
static uint32_t machinetime_to_systime(uint32_t time, uint32_t period1,
		int16_t* round_err) {
#if TIME_CONVERSION2
	...
	uint32_t result = period1 * time1 + ((time2 * per1) >> 6)
			- (uint32_t) (int32_t) *round_err;
...
static uint64_t get_system_time_and_machine(TIMER_ContextType *context,
	...
	*current_machine_time = BLUE_CTRL->CURRENT_TIME;
	...
	new_time += machinetime_to_systime(difftime, context->period1,
	...
	context->cumulative_time = context->last_calibration_time
			+ machinetime_to_systime(
					TIME_ABSDIFF(current_machine_time,
							context->last_calibration_machine_time), period,
					&(context->roundingError));
...
static void _update_xtal_startup_time(uint16_t hs_startup_time, uint32_t freq1) {
	...
	if ((__blue_RAM.BlueGlobVar.Config & 8U) == 0U)
		__blue_RAM.BlueGlobVar.wakeup_time_offset =
...
void TIMER_Init(TIMER_InitType* TIMER_InitStruct) {
	...
	if (TIMER_InitStruct->TIMER_PeriodicCalibration
			|| TIMER_InitStruct->TIMER_InitialCalibration) {
	...
	TIMER_Context.periodic_calibration =
			TIMER_InitStruct->TIMER_PeriodicCalibration;
	_update_xtal_startup_time(TIMER_Context.hs_startup_time,
			TIMER_Context.freq1);
	...
	TIMER_Context.last_calibration_machine_time =
			TIMER_Context.last_machine_time;
	TIMER_Context.last_calibration_time = 0;
	TIMER_Context.tx_cal_delay_st = DELAY_TX
			+ us_to_systime((__blue_RAM.BlueGlobVar.TxDelay1) & 0x7F);
	TIMER_Context.tx_no_cal_delay_st = DELAY_TX_NO_CAL
			+ us_to_systime((__blue_RAM.BlueGlobVar.TxDelay) & 0x7F);
	TIMER_Context.rx_cal_delay_st = DELAY_RX
			+ us_to_systime((__blue_RAM.BlueGlobVar.RcvDelay1) & 0x7F);
	TIMER_Context.rx_no_cal_delay_st = DELAY_RX_NO_CAL
			+ us_to_systime((__blue_RAM.BlueGlobVar.RcvDelay1) & 0x7F);
}
...
void TIMER_Calibrate(void) {
	if (TIMER_Context.periodic_calibration) {
		...
		_update_xtal_startup_time(ContextToUpdate.hs_startup_time,
...
uint64_t TIMER_GetFutureSysTime(uint32_t time) {
	uint32_t delta_systime = machinetime_to_systime(
			TIME_DIFF(time, TIMER_Context.last_machine_time),
...
uint64_t TIMER_GetPastSysTime(uint32_t time) {
	...
	uint64_t current_system_time = get_system_time_and_machine(&TIMER_Context,
			&current_machine_time);
	delta_systime = machinetime_to_systime(
			TIME_DIFF(current_machine_time, time), TIMER_Context.period1,
...
void TIMER_GetCurrentCalibrationData(TIMER_CalibrationType *data) {
	...
	data->last_calibration_machine_time =
			TIMER_Context.last_calibration_machine_time; /** Last machine time when calibration was performed */
}
...
uint32_t TIMER_SetWakeupTime(uint32_t delay, BOOL allow_sleep) {
	...
	BLUE_CTRL->HOST_WKUP_TIMER =
			((current_time + (delay + 8)) & TIMER_MAX_VALUE) | (1 << 24)
					| (allow_sleep ? (1 << 28) : 0);
 ...
 */
uint8_t TIMER_SetRadioTimerValue(uint32_t timeout, BOOL event_type,
		BOOL cal_req) {
	...
	relTimeout = timeout
			- (uint32_t) get_system_time_and_machine(&TIMER_Context,
					&current_time) - device_delay_st;
	...
	BLUE_CTRL->TIMEOUT = ((current_time + delay) & TIMER_MAX_VALUE)
			| destination | (enable_sleep ? (1 << 28) : 0);
 ...
 * @retval None
 */
void TIMER_GetNextTimerActivity(uint32_t* nextRadioActivity,
		uint32_t* nextHostActivity) {
...
uint32_t TIMER_MachineTimeToSysTime(uint32_t time) {
	return machinetime_to_systime(time, TIMER_Context.period1,
			&TIMER_Context.roundingError);
}
