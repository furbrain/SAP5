 ...
 */
float ADC_ConvertSingleEndedVoltage(int16_t raw_value, uint8_t channel,
		uint8_t vRef, uint8_t attenuation) {
	float pga, raw_value_f, divider;
...
{
	return (ADC_KBATT
			* (ADC_ConvertSingleEndedVoltage(raw_value, ADC_Input_AdcPin2,
 ...
 */
float ADC_ConvertTemperatureSensor(int16_t raw_value, uint8_t vRef,
		uint8_t attenuation) {
	return ((ADC_KTEMP
			* (ADC_ConvertSingleEndedVoltage(raw_value, ADC_Input_AdcPin2,
					ADC_ReferenceVoltage_0V6, ADC_Attenuation_0dB))) - ADC_CTEMP);
}
float ADC_ConvertTemperatureSensorFarenait(int16_t raw_value, uint8_t vRef,
		uint8_t attenuation) {
	return ((722.0
			* (ADC_ConvertSingleEndedVoltage(raw_value, ADC_Input_AdcPin2,
					ADC_ReferenceVoltage_0V6, ADC_Attenuation_0dB))) - 499.0);
}
...
float ADC_GetConvertedData(uint8_t DataType, uint8_t Vref) {
	...
	if (DataType == ADC_Input_AdcPin1) {
		return ADC_ConvertSingleEndedVoltage(raw_value,
				(uint8_t) ADC_Input_AdcPin1, Vref, pga_reg);
	} else if (DataType == ADC_Input_AdcPin2) {
		return ADC_ConvertSingleEndedVoltage(raw_value,
				(uint8_t) ADC_Input_AdcPin2, Vref, pga_reg);
	} else if (DataType == ADC_Input_AdcPin12
			|| DataType == ADC_Input_Internal0V60V6
...
uint8_t ADC_SwCalibration(void) {
	...
	if (ifr0_v1 == 0xFFFFFFFF && ifr0_v2 == 0xFFFFFFFF
			&& ifr0_v3 == 0xFFFFFFFF) {
		return 1;
	}
	...
	v1f = (3
			* (0.6
					- ((((float) ((int16_t) (ifr0_v1 >> 16)))
							/ ADC_FS_OSR_100_200) * ADC_VREF)));
	v2f = (3
			* (0.6
					- ((((float) ((int16_t) (ifr0_v2 >> 16)))
							/ ADC_FS_OSR_100_200) * ADC_VREF)));
	v3f = (3
			* (0.6
					- ((((float) ((int16_t) (ifr0_v3 >> 16)))
							/ ADC_FS_OSR_100_200) * ADC_VREF)));
